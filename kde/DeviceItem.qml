...
PlasmaExtras.ExpandableListItem {
    ...
    function createContent(): void {
        ...
        if (model.Battery) {
            details.push(i18n("Battery"));
            details.push(model.Battery.percentage + "%");
        }

        if (model.Modalias && model.Modalias.length > 0) {
            const m = model.Modalias;
            if (m.startsWith("usb:")) {
                const vid = m.substring(5, 9);
                const pid = m.substring(10, 14);
                const ver = m.substring(15, 19);
                const verMaj = parseInt(ver, 16) >> 8;
                const verMin = parseInt(ver, 16) & 0xFF;
                details.push(i18n("Firmware"));
                details.push(verMaj + "." + verMin + "  (VID:0x" + vid + " PID:0x" + pid + ")");
            } else {
                details.push(i18n("Modalias"));
                details.push(m);
            }
        }

        if (model.DeviceClass > 0) {
            const major = (model.DeviceClass >> 8) & 0x1F;
            const majorNames = ["Misc","Computer","Phone","LAN","Audio/Video","Peripheral","Imaging","Wearable"];
            details.push(i18n("BT Class"));
            details.push("0x" + model.DeviceClass.toString(16).toUpperCase().padStart(6, '0') +
                         " (" + (majorNames[major] || "Unknown") + ")");
        }
        ...
        if (model.HasLegacyPairing) {
            details.push(i18n("Legacy Pairing"));
            details.push(boolToString(model.HasLegacyPairing));
        }

        if (model.Uuids && model.Uuids.length > 0) {
            const uuidNames = {
                "00001124": "HID",
                "00001200": "PnP",
                "0000110e": "AVRCP",
            ...
            const names = [];
            for (let i = 0; i < model.Uuids.length; i++) {
                const short_uuid = model.Uuids[i].substring(0, 8);
                names.push(uuidNames[short_uuid] || ("0x" + short_uuid.substring(4)));
            }
            details.push(i18n("Profiles"));
